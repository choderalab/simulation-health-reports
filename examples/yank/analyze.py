...
def compute_torsion_trajectories(ncfile, filename=None):
    ...
    for iteration in range(niterations):
        for replica in range(nstates):
            # Compute torsion
            torsion = compute_torsion(numpy.array(ncfile.variables['positions'][iteration,replica,:,:]), atoms)
            # Store torsion.
            phi_kt[replica,iteration] = torsion
    ...
    if filename:
        ...
        for iteration in range(niterations):
            for replica in range(nstates):
                # Write torsion
                contents += "%8.1f" % phi_kt[replica,iteration]
            contents += "\n"
...
def check_energies(ncfile, atoms):
    ...
    u_kln_replica = numpy.transpose(energies, axes=[1,2,0])
    print "Done."
...
if __name__ == '__main__':    
    ...
    source_directory = "1-methylpyrrole/"
...
#        check_positions(ncfile)
        ...
        (Deltaf_ij, dDeltaf_ij) = estimate_free_energies(ncfile, ndiscard=nequil, g=g)
