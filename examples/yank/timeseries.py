...
#=============================================================================================
def statisticalInefficiency(A_n, B_n=None, method='fast', mintime=3):
  """
  ...
  OPTIONAL ARGUMENTS
    ...
    method (string) - 'fast': use faster (but less accurate) method to estimate correlation
       time, described in Ref. [1] (default: False), 'fft': use fft
  ...
  if method != 'fft':
    ...
    while (t < N-1):
      ...
      if method == 'fast': increment += 1
  else:  
    #Use FFT to calculate correlation function
    #Note 1: This assumes data is periodic (thus divide the ifft result by N instead f (N-t)).
    #        Maybe it's more appropriate to pad with zeros?
    #Note 2: If copies of A_n, B_n aren't made, we can save ourselves an 
    #        fft when A_n is the same as B_n by checking id(A)==id(B)
    #Note 3: is <A(n)B(n+t)> = <A(n+t)B(n)> assuming stationary process?
    
    Ak = numpy.fft.fft(A_n)
    if id(A_n) == id(B_n): Bk = Ak
    else: Bk = numpy.fft.fft(B_n)
    
    #Calculate C(t) = <A(0)B(t)> - <A><B> / Cov(A,B)
    C = ( numpy.fft.ifft(Ak*Bk.conjugate()).real/N - mu_A*mu_B ) / sigma2_AB
    C = C[1:]
    
    #Set C to zero past its first zero-point
    try:
        first_zero = numpy.where(C <= 0)[0][0]
        C[first_zero:] = 0
    except: pass

    #Sum it all up to get g
    t = 1.0 - numpy.linspace(0,1,N)
    t = t[1:]
    g = 1 + 2.0*(t*C).sum()
